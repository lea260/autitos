...
class Auto_Model extends Model
{
    ...
    public function enVenta()
    {
        ...
        try {
            ...
            $query      = $pdo->prepare('SELECT id, precio_compra, precio_venta, estado, marca, modelo, anio FROM auto WHERE estado=:enventa');
            $query->bindValue(':enventa', "enventa");
            $query->execute();
            while ($row = $query->fetch()) {
                $item               = new Auto();
                $item->id           = intval($row['id']);
                $item->precioCompra = floatval($row['precio_compra']);
                $item->precioVenta  = floatval($row['precio_venta']);
                $item->marca        = $row['marca'];
                $item->modelo       = $row['modelo'];
                $item->anio         = intval($row['anio']);
